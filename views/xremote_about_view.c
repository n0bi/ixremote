...
static void xremote_about_view_draw_vertical(Canvas* canvas, XRemoteViewModel* model) {
    ...
    canvas_draw_str_aligned(canvas, 0, 50, AlignLeft, AlignTop, "Based on: XRemote");

    elements_slightly_rounded_frame(canvas, 6, 78, 52, 33);
    canvas_draw_str_aligned(canvas, 0, 69, AlignLeft, AlignTop, "Name: IR MOTE");
    canvas_draw_str_aligned(canvas, 10, 82, AlignLeft, AlignTop, "Visual IR remote");
    canvas_draw_str_aligned(canvas, 10, 94, AlignLeft, AlignTop, "and layout editor");
}
