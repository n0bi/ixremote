...
XRemoteView* xremote_view_alloc_empty() {
    ...
    if(!remote_view) return NULL;
    remote_view->on_clear = NULL;
    remote_view->app_ctx = NULL;
    remote_view->view = NULL;
    remote_view->context = NULL;
    remote_view->owns_view = false;
    return remote_view;
}
...
void xremote_view_free(XRemoteView* rview) {
    furi_assert(rview);
    xremote_view_clear_context(rview);
    if(rview->owns_view && rview->view) view_free(rview->view);
    free(rview);
}
...
void xremote_canvas_draw_header(Canvas* canvas, ViewOrientation orient, const char* section) {
    ...
    elements_multiline_text_aligned(canvas, x, 0, align, AlignTop, "IR MOTE");
